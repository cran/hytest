...
#'
#'\code{pois_c_opt} is used to obtain a critical value to achieve a nominal error type I when we use a random sample from a Poisson distribution.
...
pois_c_opt <- function(alpha=0.1,n=150,theta0=1,c1=1e-03,c2=0.99,R=15000,delta=0.005,tolerance=0.01,max_iter=100){
  k <- 1
  cond2 <- 1
  while(cond2 > tolerance & k <= max_iter){
    c_aux <- mean(c(c1,c2))
    alpha_c_aux <- pois_errorI(c_aux,n,theta0,R)$errorI_sim
