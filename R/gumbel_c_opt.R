...
#'
#'\code{gumbel_c_opt} is used to obtain a critical value to achieve a nominal error type I when we use a random sample from a Gumbel distribution.
...
#' @param theta0 numeric, represents a location under the null hypothesis of a sample from a Gumbel distribution. Default value is 0.5.
...
gumbel_c_opt <- function(alpha=0.1,n=100,theta0=1,sigma=1,c1=1e-03,c2=0.999,R=1000,delta=0.005,tolerance=0.01,max_iter=100){
  k <- 1
  cond2 <- 1
  while(cond2 > tolerance & k <= max_iter){
    c_aux <- mean(c(c1,c2))
    alpha_c_aux <- gumbel_errorI(c=c_aux,n=n,theta0=theta0,sigma=sigma,R)$errorI_sim
