...
#' @param theta0 numeric, represents the location parameter under the null hypothesis of a sample from a Gumbel distribution. Default value is 0.
#' @param sigma numeric, represents the scale parameter of a Gumbel distribution. It is assumed known and its default value is 1.
...
#' @importFrom gamlss.dist rGU dGU GU
#' @importFrom gamlss gamlss gamlss.control
#' @export gumbel_errorI

gumbel_errorI <- function(c=1,n=150,theta0=0,sigma=1,R=15000){
  counter <- 0
  for(j in 1:R){
    muestra <- rGU(n,mu=theta0,sigma=sigma)
    result <- gamlss(muestra ~ 1,family = GU,control=gamlss.control(trace = FALSE))
    mu_est <- result$mu.coefficients
    ln_ratio <- sum(dGU(muestra,mu=theta0,sigma=sigma,log=TRUE) - dGU(muestra,mu=mu_est,sigma=sigma,log=TRUE))
    if(ln_ratio < log(c)){
      counter <- counter + 1
    }
  }
